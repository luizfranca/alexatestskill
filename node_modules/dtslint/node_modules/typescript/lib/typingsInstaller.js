...
(function (ts) {
    ...
    ts.version = ts.versionMajorMinor + ".0-dev.20171031";
})(ts || (ts = {}));
(function (ts) {
...
(function (ts) {
    ...
    function getJSDocTypeTag(node) {
        var tag = getFirstJSDocTag(node, 282);
...
(function (ts) {
    ...
    function isJSDocPropertyLikeTag(node) {
        return node.kind === 285 || node.kind === 280;
    }
...
(function (ts) {
    ...
    function isDeclaration(node) {
        if (node.kind === 145) {
            return node.parent.kind !== 283 || ts.isInJavaScriptFile(node);
        }
    ...
    function isJSDocTag(node) {
        return node.kind >= 277 && node.kind <= 285;
    }
        ...
        (function (JSDocParser) {
            ...
            function parseJSDocCommentWorker(start, length) {
                ...
                function parseParameterOrPropertyTag(atToken, tagName, target) {
                    ...
                    var result = target === 1 ?
                        createNode(280, atToken.pos) :
                        createNode(285, atToken.pos);
                ...
                function parseNestedTypeLiteral(typeExpression, name) {
                    if (typeExpression && isObjectOrObjectArrayTypeReference(typeExpression.type)) {
                        ...
                        if (children) {
                            jsdocTypeLiteral = createNode(276, start_2);
                ...
                function parseClassTag(atToken, tagName) {
                    var tag = createNode(279, atToken.pos);
                    tag.atToken = atToken;
                    tag.tagName = tagName;
                ...
                function parseTypedefTag(atToken, tagName) {
                    ...
                    if (!typeExpression || isObjectOrObjectArrayTypeReference(typeExpression.type)) {
                        ...
                        while (child = tryParse(function () { return parseChildParameterOrPropertyTag(0); })) {
                            if (!jsdocTypeLiteral) {
                                jsdocTypeLiteral = createNode(276, start_3);
                            }
                            if (child.kind === 282) {
                                if (childTypeTag) {
                                    break;
